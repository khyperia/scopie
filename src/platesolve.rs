...
use regex::Regex;
use std::{env::var, path::PathBuf, process::Command, sync::Once};
...
pub fn platesolve(tex: &CpuTexture<u16>) -> Result<(Angle, Angle)> {
    ...
    let caps = parse_regex()
        .captures(&output)
        .ok_or_else(|| failure::err_msg("Error searching solve-field output"))?;
    let ra = caps
        .get(1)
    ...
    Ok((Angle::from_degrees(ra), Angle::from_degrees(dec)))
}
